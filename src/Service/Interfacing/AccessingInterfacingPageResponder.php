...
namespace App\Bridging\Service\Interfacing;
...
#[AutoconfigureTag('app.bridging.bridge')]
final readonly class AccessingInterfacingPageResponder implements BridgeInterface
{
    private const string Target = 'interfacing';
    private const string Template = 'bridging/accessing/screen.html.twig';

    /**
     * @var array<string, array<string, mixed>>
     */
    private const array SCREEN_MAP = [
        'account.overview' => [
    ...
    {
        return $payload instanceof PageView && self::Target === $target;
    }
    ...
    public function bridge(object $payload, string $target, array $context = []): array
    {
        if (!$this->supports($payload, $target)) {
            throw new \InvalidArgumentException('Unsupported bridge payload for Accessing → Interfacing bridge.');
        }
        ...
        if (null === $spec) {
            throw new \LogicException(sprintf('No Accessing → Interfacing bridge mapping configured for "%s".', $pageView->view));
        }
    ...
    public function respond(PageView $pageView): Response
    {
        return $this->renderer->render(
            self::Template,
            [
                'screen' => $this->bridge($pageView, self::Target),
                'pageView' => $pageView,
            ],
            $pageView->statusCode,
        );
